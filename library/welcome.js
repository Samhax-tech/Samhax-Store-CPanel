...
async function welcomeBanner(avatar, name, subject, type) {
    const title = name
    const desc = (type == "welcome" ? "Welcome to " : "Has left the group ") + subject
...
async function promoteBanner(avatar, name, type) {
    const title = name
    const desc = type == "promote" ? "Have become admin" : "Has been removed as admin"
    const background = "https://img101.pixhost.to/images/642/557922982_skyzopedia.jpg"
    const welcome = await new canvafy.WelcomeLeave()
    .setAvatar(avatar)
    .setBackground("image", background)
